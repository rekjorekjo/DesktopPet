...
聊天窗口中可见的用户消息、宠物回复和系统提示会写入本地 JSONL 日志文件。
...
用户可在"搜索设置"页面配置搜索服务（Tavily / Brave Search / Exa）及其 API Key。
...
"首次对话自动搜索"选项：启用后，应用启动后首次对话时会根据角色设定自动搜索实时信息，搜索结果注入对话上下文。
